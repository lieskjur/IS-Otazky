...
% \begin{multicols}{2}
	...
	\section{Uveďte strukturu nelineární identifikace používající koncept LOLIMOT. }
	Local Linear Models Tree
	\begin{equation}
		\hat{y} = \sum_{i=1}^M \hat{y}_i \phi_i(\bm{u})
	\end{equation}
	kde $\hat{y}_i = w_{i0} + w_{i1} u_1 + w_{i2} u_2 + \dots$ jsou lokální lineární modely a $\phi_i(u)$ jejich platnostní funkce. Ty splňují
	\begin{equation}
